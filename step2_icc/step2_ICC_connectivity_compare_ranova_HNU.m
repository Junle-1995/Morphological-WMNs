...
save_path = [path '\data_connectivity_ranova_HNU'];
mkdir(save_path)
...
load('MRM_con_HNU.mat')
mkdir('data_connectivity_ranova_results_HNU')
MRM_estimate

%% collect the results
MRM_path = [path '\data_connectivity_ranova_results_HNU\Contrasts'];
for itype = 1:length(Type)
...
%% save
cd(path)
save('ICC_connectivity_compare_ranova_HNU.mat','results','post')