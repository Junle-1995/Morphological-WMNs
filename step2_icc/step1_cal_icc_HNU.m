%% calculate the ICC of each connectivity in HNU data
...
for inet = 1:length(net_type)
    for ipro = 1:length(pro_type)
        ...
        cd(path)
        cd ..
        data_path = [pwd '\step0_network_construction\' net_type{inet} '_HNU'];
        ...
        for inode = 1:node_num
            for jnode = inode+1:node_num
                ...
                results(end).icc_short(inode,jnode) = IPN_icc(data1,1,'single');
            end
        end
...
%% save
cd(path)
save('ICC_connectivity_HNU.mat','results')