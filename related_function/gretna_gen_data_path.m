function gretna_gen_data_path(Data_path, Output_path, Output_filename, Subfolder_name)
...
%    Subfolder_name (opt):
%                   The subfolder full name within inviduial folders.
%
...
if nargin == 3
    ...
    Subfolder_name = [filesep Subfolder_name];
end
...
for g = 1:NumGroup
    ...
    if g ~= NumGroup
        for i = 3:length(files)
            fprintf(fid, '%s\n', [Data_path{g} filesep files(i).name Subfolder_name]);
        end
    else
        for i = 3:length(files)
            if i ~= length(files)
                ...
                fprintf(fid, '%s', [Data_path{g} filesep files(i).name Subfolder_name]);
            end
        end
    end
end
