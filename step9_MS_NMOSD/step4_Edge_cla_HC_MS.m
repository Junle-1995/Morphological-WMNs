%% binary_classification to HC and MS with edge
roi_num = 48;
...
Group_label = [0*ones(size(data.HC,3),1);1*ones(size(data.MS,3),1);3*ones(size(data.NMO,3),1)];
...
save(['Edge_cla_HC_MS_' Method '_' C_type '_' num2str(C_thr) '_DBM.mat'],'results')