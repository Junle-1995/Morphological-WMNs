%% binary_classification to NMO and MS with edge
roi_num = 48;
...
Group_label = [3*ones(size(data.HC,3),1);1*ones(size(data.MS,3),1);0*ones(size(data.NMO,3),1)];
...
save(['Edge_cla_NMO_MS_' Method '_' C_type '_' num2str(C_thr) '_DBM.mat'],'results')